...
/// indicates whether HDMI instance already created
bool InstanceOpened0 = false;
/// true once a known resolution is detected on port A
bool KnownResolution0 = false;
/// true if activity is detected on port A
bool ActivityDetected0 = false;
/// identifies detected resolution of port A
tmdlHdmiRxResolutionID_t ResolutionID0 = -1;
...
// 0x4C 0xAE  0x19, for SEN ID - soon deprecated - checksum 0xE0)
    ...
    0x03, 0x80, 0x0C, 0x07, 0x78, 0xEE, 0xD8, 0x7D, 0xAC, 0x53, 0x3C, 0xB6, 0x21, 0x10, 0x4C, 0x4F, 0x00, 0x00, 0x00,
    0xD1, 0xC0, 0x01, 0x00, 0x01, 0x00, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x65, 0x33, 0x38,
    0x30, 0x40, 0x80, 0x17, 0x70, 0x20, 0x05, 0xC5, 0x00, 0x46, 0x78, 0x00, 0x00, 0x00, 0x1C, 0x00, 0x00, 0x00, 0xFF,
    0x00, 0x53, 0x45, 0x4E, 0x53, 0x49, 0x43, 0x53, 0x0A, 0x20, 0x20, 0x20, 0x20, 0x20, 0x00, 0x00, 0x00, 0xFC, 0x00,
    0x4f, 0x53, 0x56, 0x52, 0x20, 0x48, 0x44, 0x4B, 0x0A, 0x20, 0x20, 0x20, 0x20, 0x00, 0x00, 0x00, 0xFD, 0x00, 0x3B,
    0x3C, 0x64, 0x78, 0x11, 0x00, 0x0A, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x00, 0xF3,
#endif
    ...
    0x03, 0x80, 0x06, 0x0B, 0x78, 0xEE, 0xEE, 0x95, 0xA3, 0x54, 0x4C, 0x99, 0x26, 0x0F, 0x50, 0x54, 0x00, 0x00, 0x00,
    0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0xD5, 0x34, 0x38,
    0x52, 0x40, 0x80, 0x14, 0x70, 0x0A, 0x04, 0x42, 0x00, 0x3E, 0x6E, 0x00, 0x00, 0x00, 0x1C, 0x00, 0x00, 0x00, 0xFF,
    0x00, 0x53, 0x45, 0x4E, 0x53, 0x49, 0x43, 0x53, 0x0A, 0x20, 0x20, 0x20, 0x20, 0x20, 0x00, 0x00, 0x00, 0xFC, 0x00,
    0x4f, 0x53, 0x56, 0x52, 0x20, 0x48, 0x44, 0x4B, 0x0A, 0x20, 0x20, 0x20, 0x20, 0x00, 0x00, 0x00, 0xFD, 0x00, 0x3B,
    0x3C, 0x64, 0x78, 0x0F, 0x00, 0x0A, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x01, 0x8C,
 ...
 ******************************************************************************/
static void digitalActivityCallback0(tmdlHdmiRxEvent_t event, tmdlHdmiRxInput_t input, tmdlHdmiRxSyncType_t syncType)
{
...
static void storeResolution0(tmdlHdmiRxResolutionID_t resolutionID)
{
	...
	if ((resolutionID == TMDL_HDMIRX_VIDEORES_1280_720p_60HZ) ||
	    (resolutionID == TMDL_HDMIRX_VIDEORES_1280_1024p_60HZ) ||
#ifdef mode1024x768
	    (resolutionID == TMDL_HDMIRX_VIDEORES_1024_768p_60HZ) ||
#endif
	    (resolutionID == TMDL_HDMIRX_VIDEORES_1920_1080p_60HZ) ||
	    /* (resolutionID==TMDL_HDMIRX_VIDEORES_1600_1024p_60HZ) || */
	    (resolutionID ==
	     TMDL_HDMIRX_VIDEORES_1680_1050p_60HZ_RB))  // || (resolutionID==TMDL_HDMIRX_VIDEORES_1600_1200p_60HZ))
	{
...
static tmErrorCode_t tmExAppliConfigureConversion0(void)
{
	...
	case TMEX_APPLI_COLORSPACE_RGB:
		...
		NXP_Private_PRINTIF(tmdlHdmiRxSetColorConversionMatrix(gDlHdmiRxInstance0, TMDL_HDMIRX_CONVERSIONMATRIX_BYPASS,
		                                                       Null, &gRGBBlankingCode),
		                    __LINE__);
	...
	case TMEX_APPLI_COLORSPACE_YCBCR_422:
		...
		case TMEX_APPLI_COLORIMETRY_XVYCC:
			...
			if (gColorSpaceManagement0.resolutionType == TMEX_APPLI_HDTV_RESOLUTION)
			{
				...
				NXP_Private_PRINTIF(tmdlHdmiRxSetColorConversionMatrix(
				                        gDlHdmiRxInstance0, TMDL_HDMIRX_CONVERSIONMATRIX_CUSTOM,
				                        &TMEX_APPLI_CONVERSIONMATRIX_YCbCr709_RGBLimited, &gRGBBlankingCode),
				                    __LINE__);
			}
			else
			{
				HDMI_debug_progmem(cSMPTE170M);
				NXP_Private_PRINTIF(
				    tmdlHdmiRxSetColorConversionMatrix(
				        gDlHdmiRxInstance0, TMEX_APPLI_CONVERSIONMATRIX_YCbCr601_RGBLimited, Null, &gRGBBlankingCode),
				    __LINE__);
			}
			break;
		...
		case TMEX_APPLI_COLORIMETRY_SMPTE170_ITU601:
			...
			HDMI_debug_progmem(cSMPTE170M);
			NXP_Private_PRINTIF(
			    tmdlHdmiRxSetColorConversionMatrix(gDlHdmiRxInstance0, TMEX_APPLI_CONVERSIONMATRIX_YCbCr601_RGBLimited,
			                                       Null, &gRGBBlankingCode),
			    __LINE__);
			break;
		...
		case TMEX_APPLI_COLORIMETRY_ITU709:
			HDMI_debug_progmem(cITU709);
			NXP_Private_PRINTIF(
			    tmdlHdmiRxSetColorConversionMatrix(gDlHdmiRxInstance0, TMDL_HDMIRX_CONVERSIONMATRIX_CUSTOM,
			                                       &TMEX_APPLI_CONVERSIONMATRIX_YCbCr709_RGBLimited, &gRGBBlankingCode),
			    __LINE__);
			break;
		default:
...

{
	...
	if (ioport_get_value(Int_HDMI_A) == false)
	{
		...
		the actual source of the INT pin is the INTERRUPT register in the asynchronous registers part of the TDA19971.
		You can read this register using the ?CEC? I2C address of the chip that is 0x34 (instead of 0x48 for the ?HDMI?
		I2C address), and the register is at address 0xFC. This is a 5bit register: Bit 4 is HPD int (HDMI output of
		TDA19973) Bit 3 is Util int   (HDMI output of TDA19973) Bit 2 is RSX int (HDMI output of TDA19973) Bit 1 is Hdmi
		(corresponding to the TOP register) Bit 0 is CEC  int

		So if the INT pin signals an interrupt and TOP flag register is 0, that means that one of the 4 other interrupts
		sources is active. On a TDA19971, only CEC should be able to generate interrups.
...

{
	...
	tmbslTDA1997XConfigureEDID(gDlHdmiRxInstance0, BSLHDMIRX_EDID_NACK_OFF, BSLHDMIRX_EDID_RESET_OFF,
	                           BSLHDMIRX_EDID_PD_OFF, BSLHDMIRX_EDID_PD_DDC_OFF);
};
...

{
	...
	tmbslTDA1997XConfigureEDID(gDlHdmiRxInstance0, BSLHDMIRX_EDID_NACK_ON, BSLHDMIRX_EDID_RESET_ON,
	                           BSLHDMIRX_EDID_PD_ON, BSLHDMIRX_EDID_PD_DDC_ON);
};
...
tmErrorCode_t tmbslTDA1997XWriteI2C(tmUnitSelect_t unit, UInt16 firstRegister, UInt8 lengthData,
                                  UInt8 *pBuffer);  // definition for I2C routine
...

{
	...
	NewData = 0x80;
	tmErrorCode_t result = tmbslTDA1997XWriteI2C(0, INT_FLG_CLR_DDC, 1, &NewData);
	sprintf(Msg, "Write result: %" TM_ERROR_CODE_FORMAT, result);
	WriteLn(Msg);

	NewData = 0x02;
	result = tmbslTDA1997XWriteI2C(0, EMTP_CTRL, 1, &NewData);
	sprintf(Msg, "Write result: %" TM_ERROR_CODE_FORMAT, result);
	WriteLn(Msg);
	_delay_ms(10000);
	...
	result = tmbslTDA1997XWriteI2C(0, CMTP_CTRL, 1, &NewData);
	sprintf(Msg, "Write result: %" TM_ERROR_CODE_FORMAT, result);
	WriteLn(Msg);
	_delay_ms(10000);
...

{
	...
	// clear the interrupt flags
	NewData = 0x80;
	tmErrorCode_t result = tmbslTDA1997XWriteI2C(1, INT_FLG_CLR_DDC, 1, &NewData);
	sprintf(Msg, "Write result: %" TM_ERROR_CODE_FORMAT, result);
	WriteLn(Msg);

	NewData = 0x02;
	result = tmbslTDA1997XWriteI2C(1, EMTP_CTRL, 1, &NewData);
	sprintf(Msg, "Write result: %" TM_ERROR_CODE_FORMAT, result);
	WriteLn(Msg);
	_delay_ms(10000);
	...
	result = tmbslTDA1997XWriteI2C(1, CMTP_CTRL, 1, &NewData);
	sprintf(Msg, "Write result: %" TM_ERROR_CODE_FORMAT, result);
	WriteLn(Msg);
	_delay_ms(10000);
