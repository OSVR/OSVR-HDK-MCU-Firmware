...
from . import data_block, descriptor
from . import edid as edid_module
from . import extensions, keys
...
def _BuildBitsFromBitmaskList(
    options: Iterable[Tuple[int, str]], json_map: BoolDict
) -> int:
    """Encode a list of options into bit form for an EDID binary blob.
...
def BuildDtd(desc_json):
    ...
    if "Digital" in sync_json[keys.TYPE]:
        if "Separate" in sync_json[keys.TYPE]:
            x = 0x03
        ...
        x = int("Bipolar" in sync_json[keys.TYPE])
        y = int(sync_json["Serrations"])
...
def BuildDataBlock(db_json):
    ...
    if atype == data_block.DB_TYPE_AUDIO:
        ...
        for pref in prefs:
            if pref[keys.TYPE] == "Video Preference VIC":
                blob.append(pref["VIC"])
            elif pref[keys.TYPE] == "Video Preference DTD":
...
def BuildSad(sad_json):
    ...
    elif sad_json[keys.TYPE] == "DRA":  # A type of extension SAD
        ext_code = 0x07
...
def BuildEdid(edid_json):
    ...
    """
    ext_count = len(edid_json[keys.ROOT_EXTENSIONS])
    edid = [0] * ((ext_count + 1) * 128)

    base = edid_json[keys.ROOT_BASE]

    # Set up header
    edid[0x00:0x08] = [0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00]

    # Set version and revision
    version, revision = edid_json[keys.ROOT_VERSION].split(".")
    edid[0x12] = int(version)
    edid[0x13] = int(revision)

    BuildManufacturerInfo(edid, base[keys.BASE_MANUFACTURER_INFO])
    BuildBasicDisplay(edid, base[keys.BASE_BASIC_DISPLAY])
    BuildChromaticity(edid, base[keys.BASE_CHROMATICITY])
    BuildEstablishedTimings(edid, base[keys.BASE_ESTABLISHED_TIMING])
    BuildStandardTimings(edid, base[keys.BASE_STANDARD_TIMING])
    BuildDescriptors(edid, base[keys.BASE_DESCRIPTORS])
    BuildExtensions(edid, edid_json[keys.ROOT_EXTENSIONS])
