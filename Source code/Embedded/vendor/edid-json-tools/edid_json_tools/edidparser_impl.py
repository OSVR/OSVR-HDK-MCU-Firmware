...
VALID_TYPE_MESSAGE = (
    "Type options include: all (all info), base (base block),"
...
type_help_string = (
    "Types of information to print, listed as a single "
...
def PrintList(
    alist: Union[List[Tuple[Any, Any]], List[List[Any]]], mode: Mode, print_format: str
):
...
def GetBasicDisplay(e: edid.Edid, mode: Mode, raw_mode: RawMode):
    ...
    pix_str = (
        "Preferred timing includes native timing pixel\n  %-50s"
...
def PrintDtd(desc: descriptor.DetailedTimingDescriptor):
    ...
    blank = "%d x %d" % (desc.h_blanking_pixels, desc.v_blanking_lines)
            ...
            if db.type in (data_block.DB_TYPE_VIDEO, data_block.DB_TYPE_YCBCR420_VIDEO):
                ...
                for x in range(0, len(ads)):
                    ...
                    if ad.type == data_block.AUDIO_TYPE_LPCM:
                        ...
                        print(
                            "  %-19s %s"
                            % ("Bit depth:", tools.ListTrueOnly(ad.bit_depth))
                        )
                ...
                if if_proc.payload:
                    print(
                        "  %-25s %s"
                        % ("InfoFrame Processing Descriptor Payload:", if_proc.payload)
                    )
...
def PrintBase(e: edid.Edid, mode: Mode, raw_mode: RawMode, types: Collection[str]):
...
def CheckInvalidTypes(types, exts):
    ...
    for t in types:
        if t not in VALID_TYPES and t not in exts:
            print(("Error: %s is not a valid type. %s") % (t, VALID_TYPE_MESSAGE))
            PrintSpace()
